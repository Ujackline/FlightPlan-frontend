...
export default {
  ...
  async mounted() {
    try {
      ...
      await this.fetchAllAttendanceRecords(); // New method to fetch all records
      this.updateStats();
    } catch (error) {
  ...
  methods: {
    // New method to fetch all attendance records directly from the database
    async fetchAllAttendanceRecords() {
      try {
        const response = await apiClient.get('/flight-plan-t9/studentevents');
        if (response && response.data) {
          await this.processAttendanceRecords(response.data);
        } else {
          throw new Error("No data received from API");
        }
      } catch (error) {
        console.error("Error fetching all attendance records:", error);
        this.showMessage("Failed to load attendance records", "error");
        // Fall back to example records
        this.createDbRecordsDisplay();
      }
    },
    
    // Process the records from the database
    async processAttendanceRecords(dbRecords) {
      ...
      for (const dbRecord of dbRecords) {
        ...
        const status = this.mapStatus(dbRecord.status);
        
        // Get event name from events array
        const event = this.events.find(e => e.id == dbRecord.eventId);
        const eventName = event ? event.name : `Event ${dbRecord.eventId}`;
        
        // Create record with essential fields
        const record = {
          id: dbRecord.id,
          recordId: `${dbRecord.eventId}-${dbRecord.studentId}`,
          eventId: dbRecord.eventId.toString(),
          eventName: eventName,
          eventDate: dbRecord.createdAt || new Date().toISOString(),
          studentId: dbRecord.studentId.toString(),
          studentName: `Student ${dbRecord.studentId}`, // Will be updated
          displayStudentId: dbRecord.studentId.toString(), // Will be updated
          status: status,
          pointsEarned: dbRecord.pointsEarned || 0,
          reflectionText: dbRecord.reflectionText || ''
        };
    ...
    async fetchPendingApprovals() {
      try {
        ...
        // Process pending approvals and add them to the records
        if (pendingApprovals && pendingApprovals.length > 0) {
          for (const approval of pendingApprovals) {
            // Check if the record already exists
            const existingRecord = this.attendanceRecords.find(
              r => r.eventId === approval.eventId && r.studentId === approval.studentId
            );
            
            if (!existingRecord) {
              const newRecord = {
                id: `pending-${approval.eventId}-${approval.studentId}`,
                recordId: `${approval.eventId}-${approval.studentId}`,
              ...
              // Fetch student data asynchronously
              this.updateStudentName(newRecord);
            }
          }
    ...
    createDbRecordsDisplay() {
      ...
      const records = [
        ...
        },
        {
          ...
          recordId: "4-2",
          eventId: "4",
          eventName: this.events.find(e => e.id === 4)?.name || "Workshop",
          eventDate: "2025-04-24T00:00:00.000Z",
          studentId: "2",
          studentName: "Student 2",
          displayStudentId: "2",
    ...
    applyFilters() {
      ...
      if (this.searchQuery.trim()) {
        this.handleSearch();
        return; // handleSearch already updates stats
      }
    ...
    async confirmApproval() {
      try {
        ...
        const record = this.attendanceRecords.find(
          r => r.recordId === `${eventId}-${studentId}` || 
        ...
        // Refresh data after approving to ensure synchronization
    ...
    async rejectAttendance(record) {
      try {
        ...
        // Refresh data after rejecting to ensure synchronization
        await this.fetchAllAttendanceRecords();
      } catch (error) {
    ...
    async resetStatus(record) {
      try {
        ...
        try {
          ...
          try {
            const recordIdForDb = record.id || record.recordId;
            await apiClient.put(`/flight-plan-t9/studentevents/${recordIdForDb}`, {
              status: 'registered',
              pointsEarned: 0
            });
          } catch (updateError) {
